 ...
 * </pre>
 */
public class ModelSurfaceView implements CommonContract{
    @Override
    public void onModelTest(SimpleAdapter simpleAdapter, XToast toast, String TAG, Activity activity) {
        TxtItem t1 = new TxtItem("Diy-SurfaceView帧动画");
        t1.setCallback(s -> {
            toast.setText(s).show();
            LogUtils.i(TAG,s);
            activity.startActivity(new Intent(activity,DiyTestSurfaceViewActivity.class));
        });
